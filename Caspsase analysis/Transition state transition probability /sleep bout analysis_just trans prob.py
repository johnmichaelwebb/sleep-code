...
def plot_barplot(wt,wtName, mut,mutName, ylabel,name):
    ...
    sns.barplot(x="condition", y="cFos", data=framee,capsize=.1, errcolor = 'red',palette = ["lightgray",'lightblue'] )
    # sns.barplot(x="condition", y="cFos", data=framee,capsize=.1, color = "lightblue" )
...
def plot_barplot_4groups(wt,wtName, mut,mutName, wt_2,mut_2, ylabel,name):
    ...
    for i in range(len(wt_2)):
        labels.append(wtName + str(2))
    for i in range(len(mut_2)):
        labels.append(mutName + str(2))
...
def plot_barplot_6groups(wt,wtName, mut,mutName, wt_2,mut_2,wt_3, mut_3, ylabel,name):
    ...
    for i in range(len(wt_3)):
        labels.append(wtName + str(3))
    for i in range(len(mut_3)):
        labels.append(mutName + str(3))
    ...
    sns.swarmplot(x = "condition", y = "cFos", data = framee, palette = ["black",'darkblue'], alpha = 1, s = 5)
    sns.despine()
...
def plotHist(wt, mut, xname, yname, title, wtname, mutname, BIN = 30):
    ...
    plt.show()
    return
def plotPerc(wtAVG, wtSTD, mutAVG, mutSTD,name, wt_name, mut_name): 
    ...
    plt.errorbar(x_axis, wtAVG, yerr = wtSTD, c = 'black', fmt = '-o',linewidth = 3, markersize = 8, label = wt_name)
    plt.errorbar(x_axis, mutAVG, yerr = mutSTD, c = 'darkblue', fmt = '-o',linewidth = 3, markersize = 8, label = mut_name)
    # plt.errorbar(x_axis, total_avg_mut2, yerr = total_std_mut2, fmt = '-o',linewidth = 3, markersize = 8, label = mut2_name)
    plt.xlabel('ZT', fontsize = globalFont)
    plt.ylabel('% by hr',fontsize = globalFont)
    plt.xticks(ticks = [0, 6, 12, 18,80],labels = ['0','6','12','18','24'])
    ...
    ax1.add_artist(Line2D((xmin, xmax), (ymin, ymin), color='black', linewidth=2))
    ax1.add_artist(Line2D((xmin, xmin), (ymin, ymax), color='black', linewidth=2))  
    ax1.tick_params(labelsize=globalFont*.6)  
    ax1.axvspan(STARTLIGHT,ENDLIGHT, color = 'lemonchiffon')
    ax1.axvspan(ENDLIGHT,24, color = 'gainsboro')
...
wt_wake_to_wake, wt_wake_to_NREM, wt_wake_to_REM, wt_NREM_to_wake, wt_NREM_to_NREM, wt_NREM_to_REM, wt_REM_to_wake, wt_REM_to_NREM, wt_REM_to_REM = prob_state_all(WT)


plotDots(wt_wake_to_wake, WT_NAME, mut_wake_to_wake, MUTANT_NAME, 'Transition prob (%)', 'Wake to wake')
wake_to_wake_ttest = st.ttest_ind(wt_wake_to_wake,mut_wake_to_wake, equal_var = False)
wake_to_wake_ttest = wake_to_wake_ttest[1]


plotDots(wt_wake_to_NREM, WT_NAME, mut_wake_to_NREM, MUTANT_NAME, 'Transition prob (%)', 'Wake to NREM')
wake_to_NREM_ttest = st.ttest_ind(wt_wake_to_NREM,mut_wake_to_NREM, equal_var = False)
wake_to_NREM_ttest = wake_to_NREM_ttest[1]

plotDots(wt_wake_to_REM, WT_NAME, mut_wake_to_REM, MUTANT_NAME, 'Transition prob (%)', 'Wake to REM')
wake_to_REM_ttest = st.ttest_ind(wt_wake_to_REM,mut_wake_to_REM, equal_var = False)
wake_to_REM_ttest = wake_to_REM_ttest[1]


plotDots(wt_NREM_to_REM, WT_NAME, mut_NREM_to_REM, MUTANT_NAME, 'Transition prob (%)', 'NREM to REM')
NREM_to_REM_ttest = st.ttest_ind(wt_NREM_to_REM,mut_NREM_to_REM, equal_var = False)
NREM_to_REM_ttest = NREM_to_REM_ttest[1]

plotDots(wt_NREM_to_NREM, WT_NAME, mut_NREM_to_NREM, MUTANT_NAME, 'Transition prob (%)', 'NREM to NREM')
NREM_to_NREM_ttest = st.ttest_ind(wt_NREM_to_NREM,mut_NREM_to_NREM, equal_var = False)
NREM_to_NREM_ttest = NREM_to_NREM_ttest[1]

plotDots(wt_NREM_to_wake, WT_NAME, mut_NREM_to_wake, MUTANT_NAME, 'Transition prob (%)', 'NREM to Wake')
NREM_to_wake_ttest = st.ttest_ind(wt_NREM_to_wake,mut_NREM_to_wake, equal_var = False)
NREM_to_wake_ttest = NREM_to_wake_ttest[1]


plotDots(wt_REM_to_wake, WT_NAME, mut_REM_to_wake, MUTANT_NAME, 'Transition prob (%)', 'REM to Wake')
REM_to_wake_ttest = st.ttest_ind(wt_REM_to_wake,mut_REM_to_wake, equal_var = False)
REM_to_wake_ttest = REM_to_wake_ttest[1]


plotDots(wt_REM_to_REM, WT_NAME, mut_REM_to_REM, MUTANT_NAME, 'Transition prob (%)', 'REM to REM')
REM_to_REM_ttest = st.ttest_ind(wt_REM_to_REM,mut_REM_to_REM, equal_var = False)
REM_to_REM_ttest = REM_to_REM_ttest[1]

plotDots(wt_REM_to_NREM, WT_NAME, mut_REM_to_NREM, MUTANT_NAME, 'Transition prob (%)', 'REM to NREM')
...
# plotDots(wt_wake_totalTime,WT_NAME, mut_wake_totalTime, MUTANT_NAME, 'Time (hr)', 'Total wake time')
# plotDots(wt_NREM_totalTime,WT_NAME, mut_NREM_totalTime, MUTANT_NAME, 'Time (hr)', 'Total NREM time')
# plotDots(wt_REM_totalTime,WT_NAME, mut_REM_totalTime, MUTANT_NAME, 'Time (m)', 'Total REM time')
...
# plotPerc(wt_wake_perc_avg, wt_wake_perc_std,mut_wake_perc_avg,mut_wake_perc_std, 'Wake percentage by hour',WT_NAME, MUTANT_NAME)
# plotPerc(wt_NREM_perc_avg, wt_NREM_perc_std,mut_NREM_perc_avg,mut_NREM_perc_std, 'NREM percentage by hour',WT_NAME, MUTANT_NAME)
# plotPerc(wt_REM_perc_avg, wt_REM_perc_std,mut_REM_perc_avg,mut_REM_perc_std, 'REM percentage by hour',WT_NAME, MUTANT_NAME)
...
# plotDots(wt_light_REM_totalTime,WT_NAME, mut_light_REM_totalTime, MUTANT_NAME, 'time (m)', 'Total light REM time')
...
# plot_barplot(wt_REM_len_avg,WT_NAME, mut_REM_len_avg, MUTANT_NAME, 'Bout len (s)', 'Average REM bout length')
...
# plot_barplot_4groups(wt_wake_len_avg_min,WT_NAME, mut_wake_len_avg_min, MUTANT_NAME, wt_NREM_len_avg_min,mut_NREM_len_avg_min,'Bout len (m)', 'Average wake bout length + NREM')
