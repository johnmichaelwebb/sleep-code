...
def plotPercCI(wake, wakeCI, NREM ,NREMCI,  REM,REMCI, sec, name,globalFont, global_name, inhibitory, stim_length):
    ax1 = plt.axes(frameon=False)
    ...
    plt.ylim(0,102)
    plt.savefig(plot_dir+name +'opto_stim.pdf')
    plt.show()
    return

def create_sec(bout_per_min,beforeAndAfter,stim_length,bout_length):
...
def plotPercSEM(wake, wakeSEM, NREM,NREMSEM,  REM,REMSEM, sec, name, globalFont, global_name, inhibitory, stim_length):
    ax1 = plt.axes(frameon=False)
